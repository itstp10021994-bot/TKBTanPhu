...
with top_c2:
    if st.button("↺ Khôi phục dữ liệu mẫu", use_container_width=True):
        for key, default in DEFAULTS.items():
            st.session_state[key] = default.copy() if hasattr(default, "copy") else default
        for key, default in DEFAULT_CONSTRAINT_TOGGLES.items():
            st.session_state[key] = default
...
# =======================================================================
# TAB CHÍNH — chia nhỏ giao diện cho gọn, dễ theo dõi hơn thay vì 1 trang dài
# =======================================================================
tab_cfg, tab_data, tab_activities, tab_constraints, tab_result, tab_exam = st.tabs([
    "⚙️ Cấu hình chung",
    "🏢 Tổ / GV / Lớp / Phòng",
    "📚 Môn học & phân công",
...
with tab_cfg:
    ...
    with c1:
        num_days = st.selectbox("Số ngày học/tuần", [5, 6], index=1)
    with c2:
    ...
    max_seconds = st.slider("Thời gian tối đa cho solver tìm lời giải (giây)", 5, 120, 30)
    school_name = st.text_input("Tên trường (hiển thị trên PDF xuất ra, không bắt buộc)", value="")
    st.session_state.tuan_bat_dau = st.date_input(
        "Ngày bắt đầu tuần (Thứ 2) — để hiện ngày thực tế trên thời khoá biểu",
